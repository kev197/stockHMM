# stockHMM
In many real world scenarios it is of interest to model some observable signal to better understand its underlying properties. For example, in the realm of speech processing we may have a spectrum of frequencies of some word or phrase captured over a period of time. Discretize these spectrums into finite sized continuous observation vectors using a decompressing method and denote the set of these vectors as the stochastic process {O_t}. It is of interest to identify the actual word or phrase spoken given the sequence of observations. For example, in the case of determining a singular word we may choose to decompose it into a set of time-indexed spectral vectors {O_1, O_2, ... , O_T}, then denote the underlying stochastic process that produced those signals as being some chain of phonems or syllables corresponding to pieces of that word. The application of Markov Models aims to both capture 1. the number of phonems (or whatever the "states" are) and 2. a characterization of what these states are doing over time. We can do so by assigning the underlying process some number of distinct states with associated state transitions i.e. a Markov Model. Then, we attach probability distributions to each state responsible for generating the observed signals (in this case the stochastic process of spectral vectors {O_t}), as well as an initial state distribution. It remains to adjust the parameters of this model to maximize the likelihood of the model producing the observations. This is done by generalizing EM from statistics to the HMM and solving a constrained optimization problem for a lower bound on the log likelihood. 
