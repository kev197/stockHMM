# stockHMM
Using hidden markov models to delineate profitable/unprofitable latent market states. 

In many real world scenarios, it is of interest to analyze a signal and determine some kind of underlying state. For example, in the realm of speech processing we have a spectrum of frequencies captured over a period of time, such as a word or phrase. Then, it is of interest to determine the actual word or phrase spoken. Let the spectrum be the stochastic process of observation vectors {O_t}. An important problem is to probabilistically model the production of those observable signals, which in itself may be a stochastic process. For example, in the case of determining a singular word we may choose to decompose the word into a set of time-indexed spectral vectors {O_1, O_2, ... , O_T}, then denote the underlying stochastic process that produced those signals as being some chain of phonems or syllables corresponding to pieces of that word. The application of Markov Models aims to both capture 1. the number of "states" in this underlying process 2. a characterization of the process. We do so by assigning the underlying process as some number of states with associated transitions i.e. a Markov Model. We then generate a distribution for the initial state, as well as attach probability distributions to each state that generate the observable signals. It remains to adjust the parameters of this model to better fit the observation signal using an EM approach. 
